...
The BeatPass API supports **Bearer token authentication** for approved developer integrations.

<Warning>
  API access is **not self-service**. Developer tokens are granted on a case-by-case basis. To request API access, contact [**contact@beatpass.ca**](mailto:contact@beatpass.ca).
</Warning>
...
<AccordionGroup>
  <Accordion title="401 Unauthenticated" icon="circle-xmark">
    <Badge color="red">401</Badge> Token missing, invalid, or expired.
  ...
  <Accordion title="403 Forbidden" icon="ban">
    <Badge color="orange">403</Badge> Your account lacks permission for this resource.
