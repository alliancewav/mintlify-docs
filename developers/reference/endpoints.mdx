...
<Tip>
  Use the playground controls on the right to send live requests. Base URL: `https://open.beatpass.ca/api/v1`. Authenticated calls accept `Authorization: Bearer <token>` or your logged-in session cookies.
</Tip>

<CardGroup cols={3}>
  <Card title="REST base" icon="globe">
    `https://open.beatpass.ca/api/v1`
  </Card>
  <Card title="Auth header" icon="key">
    `Authorization: Bearer &lt;token&gt;` (most endpoints also honor session cookies)
  </Card>
  <Card title="Playground tips" icon="wand-magic-sparkles">
    Provide realistic IDs and optional query params, then hit <strong>Try it</strong>
     to see real responses or errors.
  </Card>
</CardGroup>

<AccordionGroup>
  <Accordion title="List tracks (GET /tracks)" icon="list">
    <Badge color="blue">GET</Badge>
     <Badge color="gray">/tracks</Badge>
     <Badge color="green">Optional auth</Badge>

    <ParamField query="page" type="integer">
      Page number (defaults to 1).
    </ParamField>
    <ParamField query="perPage" default={20} type="integer">
      Page size (max 50).
    </ParamField>
    <ParamField query="q" type="string">
      Search term to filter tracks by title, artist, or tags.
    </ParamField>
    <ParamField query="genre" type="string">
      Optional genre slug filter.
    </ParamField>
    <RequestExample>

    ```bash cURL
    curl "https://open.beatpass.ca/api/v1/tracks?perPage=20&q=lofi" \
      -H "Authorization: Bearer YOUR_TOKEN"
    ```

    
    ```javascript fetch
    const res = await fetch("https://open.beatpass.ca/api/v1/tracks?perPage=20", {
    ...
    {
      "data": [
        {"id": 123, "title": "Midnight Chase", "artist": "ProducerName"},
        {"id": 456, "title": "Skyline", "artist": "Another Producer"}
      ],
  ...
  <Accordion title="Track details (GET /tracks/{track})" icon="music">
    <Badge color="blue">GET</Badge>
     <Badge color="gray">/tracks/{track}</Badge>
     <Badge color="green">Optional auth</Badge>
    ...
    ```bash cURL
    curl "https://open.beatpass.ca/api/v1/tracks/123" \
      -H "Authorization: Bearer YOUR_TOKEN"
    ```
  ...
  <Accordion title="Calculate purchase price (POST /tracks/{track}/purchase/calculate-price)" icon="calculator">
    <Badge color="blue">POST</Badge>
     <Badge color="gray">/tracks/{track}/purchase/calculate-price</Badge>
     <Badge color="red">Auth required</Badge>

    <ParamField path="track" type="integer" required>
      Track ID to price.
    </ParamField>
    <ParamField body="license_type" type="string" required placeholder="non_exclusive">
      License type to quote: `non_exclusive` or `exclusive`.
    </ParamField>
    <ParamField body="coupon_code" type="string">
      Optional coupon to apply before checkout.
    </ParamField>
    <RequestExample>
  ...
  <Accordion title="Initiate purchase (POST /tracks/{track}/purchase/initiate)" icon="credit-card">
    <Badge color="blue">POST</Badge>
     <Badge color="gray">/tracks/{track}/purchase/initiate</Badge>
     <Badge color="red">Auth required</Badge>
  ...
  <Accordion title="Check license status (GET /tracks/{track}/check-license)" icon="shield-check">
    <Badge color="blue">GET</Badge>
     <Badge color="gray">/tracks/{track}/check-license</Badge>
     <Badge color="red">Auth required</Badge>
  ...
  <Accordion title="Insights snapshot (GET /insights-report)" icon="chart-line">
    <Badge color="blue">GET</Badge>
     <Badge color="gray">/insights-report</Badge>
     <Badge color="green">Optional auth</Badge>

    <ParamField query="startDate" type="string">
      ISO 8601 start date (defaults to backend range).
    </ParamField>
    <ParamField query="endDate" type="string">
      ISO 8601 end date.
    </ParamField>
    <ParamField query="metrics" type="string">
      Comma-separated metrics (`plays,devices,browsers,platforms,locations,tracks,artists,albums,users`).
    </ParamField>
    <ParamField query="model" type="string">
      Report target, e.g., `track:123` or `album:45`.
    </ParamField>
    <RequestExample>

    ```bash cURL
    curl "https://open.beatpass.ca/api/v1/insights-report?metrics=plays,devices&model=track:123" \
      -H "Authorization: Bearer YOUR_TOKEN"
    ```
...
<Note>
  Looking for every endpoint? Jump to the full catalog below.
</Note>
...
| Method | Endpoint           | Auth | Description                                                   |
| ------ | ------------------ | ---- | ------------------------------------------------------------- |
| GET    | `/insights-report` | 🔓\* | On-demand analytics report generated by `BuildInsightsReport` |
...
<Tip>
  🔒 Authentication is required when requesting data for protected resources. Track, album, and artist scoped reports honor authorization checks, and global `track_play` analytics require admin access. No additional rate limits are applied beyond platform defaults.
</Tip>
...
| Method | Endpoint                                | Auth | Description                                              |
| ------ | --------------------------------------- | ---- | -------------------------------------------------------- |
| GET    | `/artist/profile`                       | 🔒   | Current artist profile tied to the authenticated user    |
| GET    | `/artist/dashboard-stats`               | 🔒   | Dashboard cards for plays, contribution, and top track   |
| GET    | `/artist/track-performance`             | 🔒   | Recent tracks with plays, contribution, and upload dates |
| GET    | `/artist/upload-insights`               | 🔒   | Upload velocity, best month, and cadence recommendations |
| GET    | `/artist/{artist}/collaborative-tracks` | 🔒   | Co-produced tracks and collaborators for the artist      |
...
| Method | Endpoint                                            | Auth | Description                                          |
| ------ | --------------------------------------------------- | ---- | ---------------------------------------------------- |
| GET    | `/artists/{artistId}/collaboration-recommendations` | 🔓   | Genre-matched artists you have not collaborated with |
...
| Method | Endpoint                            | Auth | Description                                            |
| ------ | ----------------------------------- | ---- | ------------------------------------------------------ |
| GET    | `/artists/{artist}/check-ownership` | 🔓   | Determine whether an artist profile is already claimed |
