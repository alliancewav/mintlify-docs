...
The Messaging API enables direct and group conversations between BeatPass users. It supports real-time features including typing indicators and read receipts via Pusher broadcasting.

<Info>
  **Base URL:** `https://open.beatpass.ca/api/v1/messaging`

  **Authentication:** All messaging endpoints require an authenticated session.
</Info>
...
<Note>
  Typing indicators are broadcast in real-time via Pusher. The REST endpoints are fallbacks for environments without WebSocket support.
</Note>
...
<CardGroup cols={2}>
  <Card title="Authentication" icon="key" href="/developers/auth">
    Session-based authentication for messaging.
  </Card>
