...
- **Status** — Paid, Queued, Sending, Failed, or Excluded

<Info>
  Subscription payouts can stay **Queued** briefly if BeatPass is waiting for platform funds to settle or be replenished. When that happens, the system resumes them automatically once Stripe reports the required balance is available.
</Info>
...
| Filter | Options |
|--------|---------|
| **Quick Filters** | All, Sales, Subscriptions, Paid, Pending, Failed |
...
<Accordion title="A subscription payout is stuck in Queued">
  **What it means:**
  - Your split was calculated successfully
  - BeatPass is waiting for platform funds to be available in Stripe
  - The payout will resume automatically when funds are available
  
  **What to do:** Usually nothing. If your Stripe Connect account is still active, BeatPass will retry automatically and update the status to **Sending** and then **Paid** once the transfer goes through.
</Accordion>
