...
$$
\text{Contribution Value} = \text{Time Decay} \times \text{Popularity Boost} \times \text{Collaboration Split}
$$
...
```mermaid
flowchart LR
    A[Time Decay\n1.0 → 0] --> D[Contribution\nValue]
    B[Popularity Boost\n1x → 3x] --> D
    C[Collaboration Split\n÷ producers] --> D
```
...
**Final contribution per producer:** $0.70 \times 2.14 \times 0.5 = \mathbf{0.75}$ **points**
