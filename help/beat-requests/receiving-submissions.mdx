...
<Note>
  Submissions are existing tracks from producers' catalogs, not custom-made beats. This allows for quick turnaround during the request window.
</Note>
...
<AccordionGroup>
  <Accordion title="Can producers submit more than one beat?">
    Yes. A producer can submit up to 5 different tracks to the same request. Each specific track can only be submitted once, and duplicate audio (tracks with matching fingerprints) is automatically blocked.
  </Accordion>
